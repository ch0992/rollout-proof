---
task_id: P0-035
title: "10회 반복 재현 harness"
status: backlog
size: S
milestone: Phase 0
epic: P0-E6
issue: https://github.com/ch0992/rollout-proof/issues/41
branch: "feat/41-p0-035-repeat-harness"
work_order_version: 1
evaluation_document: ../evaluations/P0-035-evaluation.md
---

# P0-035 작업지시서: 10회 반복 재현 harness

## 1. 목적

10회 반복 재현 harness을 독립적으로 구현하고 후속 Task가 의존할 수 있는 검증된 산출물을 만든다.

## 2. 선행조건

- [Phase 0 dependency graph](../phase-0-task-breakdown.md)의 선행 Task가 PASS 상태여야 한다.
- Issue URL과 base commit을 작업 전에 기록한다.

## 3. 참조 계약

- Requirement ID: `TEST-E2E-005`
- [엔지니어링 구현 스펙](../implementation-spec.md)
- 관련 세부 section은 Issue 생성 시 requirement와 함께 고정한다.

## 4. 작업 범위

- 정상/결함 scenario 각 10회 실행
- run별 artifact와 aggregate 비율

## 5. 제외 범위

- 통계적 release 기준 확정
- flaky 자동 retry

## 6. 예상 변경 파일

- `test/e2e/repeat.sh`
- `test/e2e/repeat_test.go`
- `docs/experiments/phase-0-results.md`

예상하지 않은 주요 파일이 2개 이상 필요하면 구현을 중단하고 Issue를 재검토한다.

## 7. Acceptance Criteria

- [ ] AC-1: 정확히 요청한 count만큼 독립 run을 수행한다.
- [ ] AC-2: run별 exit/result/artifact 경로를 보존한다.
- [ ] AC-3: 정상 누락률과 결함 탐지율을 계산한다.
- [ ] AC-4: 실패 run을 자동으로 PASS까지 retry하지 않는다.

## 8. 구현 절차

1. 실패 test, fixture 또는 inspection check를 먼저 준비한다.
2. 예상 파일 범위에서 최소 구현을 수행한다.
3. targeted 검증과 race/static 검사를 실행한다.
4. 제외 범위 침범과 dependency 증가를 확인한다.
5. AC별 evidence와 commit SHA를 평가서에 전달한다.

## 9. 검증 명령

```bash
go test ./test/e2e -run TestRepeatHarness
test/e2e/repeat.sh --scenario normal --count 10
test/e2e/repeat.sh --scenario endpoint-gap --count 10
```

## 10. 산출물

- 지정 source/test/document와 검증 결과
- AC별 evidence와 평가 대상 commit SHA

## 11. 형상관리 계약

- Branch: `feat/41-p0-035-repeat-harness`
- Commit prefix: `[P0-035]`
- PR title: `[P0-035] 10회 반복 재현 harness`
- PR은 `Closes #41` 및 작업지시서/평가서 링크를 포함한다.
- 평가서 PASS와 required CI PASS 전에는 merge하지 않는다.

## 12. 완료 보고

```text
Implemented:
Changed files:
Verification:
Acceptance evidence:
Known limitations:
Commit SHA:
```

