...
branch: "feat/25-p0-019-probe-types"
work_order_version: 1
evaluation_document: ../evaluations/P0-019-evaluation.md
---

# P0-019 작업지시서: HTTP Probe domain type

## 1. 목적

HTTP Probe domain type을 독립적으로 구현하고 후속 Task가 의존할 수 있는 검증된 산출물을 만든다.
...
- Requirement ID: `ENG-PROBE-001`
- [엔지니어링 구현 스펙](../implementation-spec.md)
- 관련 세부 section은 Issue 생성 시 requirement와 함께 고정한다.

## 4. 작업 범위

- request spec/result/phase timing type
- connection reuse와 error category 표현
...
- [ ] AC-1: method는 GET/HEAD만 유효하다.
- [ ] AC-2: result가 status, latency, connection reuse, typed failure를 표현한다.
- [ ] AC-3: URL userinfo/query는 domain evidence에 원문 저장되지 않는다.
- [ ] AC-4: invalid spec은 request 전에 typed error다.
...
```bash
go test ./internal/probe -run 'Test.*Spec|Test.*Result'
...
- Branch: `feat/25-p0-019-probe-types`
- Commit prefix: `[P0-019]`
- PR title: `[P0-019] HTTP Probe domain type`
- PR은 `Closes #25` 및 작업지시서/평가서 링크를 포함한다.
