...
# P0-020 작업지시서: Bounded probe scheduler

## 1. 목적

Bounded probe scheduler을 독립적으로 구현하고 후속 Task가 의존할 수 있는 검증된 산출물을 만든다.
...
- bounded queue와 worker limit
- schedule lag event

## 5. 제외 범위

- HTTP transport
- multi-probe weight policy

## 6. 예상 변경 파일

- `internal/probe/scheduler.go`
- `internal/probe/scheduler_test.go`

예상하지 않은 주요 파일이 2개 이상 필요하면 구현을 중단하고 Issue를 재검토한다.

## 7. Acceptance Criteria

- [ ] AC-1: fake clock으로 지정 interval job을 만든다.
- [ ] AC-2: queue가 가득 차도 무한 backlog를 만들지 않는다.
- [ ] AC-3: lag/drop을 diagnostic result로 노출한다.
- [ ] AC-4: context 취소 후 worker가 종료된다.
