...
# P0-014 작업지시서: WATCH EOF reconnect와 backoff

## 1. 목적

WATCH EOF reconnect와 backoff을 독립적으로 구현하고 후속 Task가 의존할 수 있는 검증된 산출물을 만든다.
...
- EOF/일시 network 오류 재연결
- 250ms~10s full-jitter exponential backoff
- 재연결 diagnostic event
...
- `internal/kube/watch/reconnect.go`
- `internal/kube/watch/reconnect_test.go`

예상하지 않은 주요 파일이 2개 이상 필요하면 구현을 중단하고 Issue를 재검토한다.

## 7. Acceptance Criteria

- [ ] AC-1: EOF 후 마지막 resourceVersion으로 재연결한다.
- [ ] AC-2: backoff가 250ms에서 시작해 10s를 넘지 않는다.
- [ ] AC-3: fake clock/random source로 test가 결정적이다.
- [ ] AC-4: 인증/권한 오류는 무한 retry하지 않는다.
