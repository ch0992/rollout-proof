...
# P0-028 작업지시서: 최소 Terminal timeline renderer

## 1. 목적

최소 Terminal timeline renderer을 독립적으로 구현하고 후속 Task가 의존할 수 있는 검증된 산출물을 만든다.
...
- canonical event의 한 줄 렌더링
- writer 주입과 stable snapshot
...
- `internal/report/terminal/renderer.go`
- `internal/report/terminal/renderer_test.go`

예상하지 않은 주요 파일이 2개 이상 필요하면 구현을 중단하고 Issue를 재검토한다.

## 7. Acceptance Criteria

- [ ] AC-1: 각 event의 elapsed/source/kind가 한 줄에 표시된다.
- [ ] AC-2: renderer가 stdout을 직접 참조하지 않는다.
- [ ] AC-3: 동일 event stream의 snapshot이 안정적이다.
- [ ] AC-4: payload의 secret 후보가 redaction 없이 출력되지 않는다.
