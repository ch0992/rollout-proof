...
# P0-028 평가서: 최소 Terminal timeline renderer

## 1. 평가 계약

[작업지시서](../tasks/P0-028-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.

## 2. 평가 대상

- Work order version: 1
- Requirement ID: `ENG-REPORT-003`
- Issue/PR/commit SHA: 구현 후 기록
- 허용 주요 파일: `internal/report/terminal/renderer.go`, `internal/report/terminal/renderer_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 각 event의 elapsed/source/kind가 한 줄에 표시된다. | 미평가 | |
| AC-2 | command + 관련 inspection | renderer가 stdout을 직접 참조하지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | 동일 event stream의 snapshot이 안정적이다. | 미평가 | |
| AC-4 | command + 관련 inspection | payload의 secret 후보가 redaction 없이 출력되지 않는다. | 미평가 | |
...
```bash
go test ./internal/report/terminal
go test ./internal/report/terminal -run TestSnapshot
```
