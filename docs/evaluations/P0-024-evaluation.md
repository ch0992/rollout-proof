...
work_order: ../tasks/P0-024-work-order.md
evaluation_version: 1
verdict: null
---

# P0-024 평가서: HTTP status policy

## 1. 평가 계약

[작업지시서](../tasks/P0-024-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/probe/policy/status.go`, `internal/probe/policy/status_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 기본값은 200~299만 성공이다. | 미평가 | |
| AC-2 | command + 관련 inspection | 사용자 allowlist가 기본값을 명시적으로 대체한다. | 미평가 | |
| AC-3 | command + 관련 inspection | 중복/역전/범위 밖 status 설정을 거부한다. | 미평가 | |
| AC-4 | command + 관련 inspection | 같은 입력은 항상 같은 판정이다. | 미평가 | |
