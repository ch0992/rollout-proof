...
work_order: ../tasks/P0-010-work-order.md
evaluation_version: 1
verdict: null
---

# P0-010 평가서: Atomic report writer 구현

## 1. 평가 목적

[P0-010 작업지시서](../tasks/P0-010-work-order.md)의 계약을 구현 설명과 독립적으로 검증한다. 검증 방법과 PASS 조건은 구현 전에 고정하며 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/report/writer.go`, `internal/report/writer_test.go`
...
|---|---|---|---|---|
| AC-1 | `go test ./internal/report -run 'Test.*Atomic'` 및 관련 code inspection | 성공 시 target에 전체 byte가 기록된다. | 미평가 | |
| AC-2 | `go test -race ./internal/report` 및 관련 code inspection | write 실패 시 기존 target 내용이 보존된다. | 미평가 | |
| AC-3 | `go test -race ./internal/report` 및 관련 code inspection | 실패 후 temporary file이 남지 않는다. | 미평가 | |
| AC-4 | `go test -race ./internal/report` 및 관련 code inspection | permission/rename 오류가 wrapping되어 caller에 반환된다. | 미평가 | |
| AC-5 | `go test -race ./internal/report` 및 관련 code inspection | unit test가 실제 temporary directory에서 동작한다. | 미평가 | |
