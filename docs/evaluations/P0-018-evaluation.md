...
# P0-018 평가서: ADR-0004 LIST/WATCH supervisor 결정

## 1. 평가 계약

[작업지시서](../tasks/P0-018-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `docs/adr/0004-list-watch-supervisor.md`, `docs/README.md`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | Context/Decision/Alternatives/Consequences가 있다. | 미평가 | |
| AC-2 | command + 관련 inspection | initial LIST 후 WATCH 순서를 명시한다. | 미평가 | |
| AC-3 | command + 관련 inspection | 410 gap을 숨기지 않는 정책을 명시한다. | 미평가 | |
