...
work_order: ../tasks/P0-012-work-order.md
evaluation_version: 1
verdict: null
---

# P0-012 평가서: Deployment selector discovery

## 1. 평가 계약

[작업지시서](../tasks/P0-012-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/kube/discovery/deployment.go`, `internal/kube/discovery/deployment_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | namespace/name으로 Deployment를 조회한다. | 미평가 | |
| AC-2 | command + 관련 inspection | empty selector를 거부한다. | 미평가 | |
| AC-3 | command + 관련 inspection | 관련 ReplicaSet은 owner UID로만 연결한다. | 미평가 | |
| AC-4 | command + 관련 inspection | revision annotation 파싱 오류를 evidence가 있는 typed error로 반환한다. | 미평가 | |
