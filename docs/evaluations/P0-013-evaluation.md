...
# P0-013 평가서: Generic LIST/WATCH supervisor

## 1. 평가 계약

[작업지시서](../tasks/P0-013-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/kube/watch/supervisor.go`, `internal/kube/watch/supervisor_test.go`, `internal/kube/watch/fake_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | LIST item이 watch event보다 먼저 전달된다. | 미평가 | |
| AC-2 | command + 관련 inspection | WATCH는 LIST resourceVersion에서 시작한다. | 미평가 | |
| AC-3 | command + 관련 inspection | bookmark가 진행 resourceVersion을 갱신한다. | 미평가 | |
| AC-4 | command + 관련 inspection | context 취소 시 watch와 output이 종료된다. | 미평가 | |
| AC-5 | command + 관련 inspection | bounded output이 무제한 memory를 사용하지 않는다. | 미평가 | |
