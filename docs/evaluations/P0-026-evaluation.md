...
# P0-026 평가서: Watch와 Probe producer를 Event bus에 연결

## 1. 평가 계약

[작업지시서](../tasks/P0-026-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/app/observe/pipeline.go`, `internal/app/observe/pipeline_test.go`, `internal/timeline/bus.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | watch와 probe event가 하나의 output stream에 도달한다. | 미평가 | |
| AC-2 | command + 관련 inspection | 한 producer 오류가 root context를 취소한다. | 미평가 | |
| AC-3 | command + 관련 inspection | channel close 순서가 send-on-closed panic을 만들지 않는다. | 미평가 | |
| AC-4 | command + 관련 inspection | bounded drain 후 종료한다. | 미평가 | |
