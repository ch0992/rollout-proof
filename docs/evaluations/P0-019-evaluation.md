...
# P0-019 평가서: HTTP Probe domain type

## 1. 평가 계약

[작업지시서](../tasks/P0-019-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/probe/result.go`, `internal/probe/result_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | method는 GET/HEAD만 유효하다. | 미평가 | |
| AC-2 | command + 관련 inspection | result가 status, latency, connection reuse, typed failure를 표현한다. | 미평가 | |
| AC-3 | command + 관련 inspection | URL userinfo/query는 domain evidence에 원문 저장되지 않는다. | 미평가 | |
| AC-4 | command + 관련 inspection | invalid spec은 request 전에 typed error다. | 미평가 | |
