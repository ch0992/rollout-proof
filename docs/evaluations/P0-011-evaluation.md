...
status: planned
issue: https://github.com/ch0992/rollout-proof/issues/17
pull_request: null
evaluated_commit: null
work_order: ../tasks/P0-011-work-order.md
evaluation_version: 1
verdict: null
---

# P0-011 평가서: Kubeconfig와 REST config loader

## 1. 평가 계약

[작업지시서](../tasks/P0-011-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/kube/client/config.go`, `internal/kube/client/config_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | flag가 config 기본값보다 우선한다. | 미평가 | |
| AC-2 | command + 관련 inspection | namespace 미지정 시 context namespace, 없으면 default를 사용한다. | 미평가 | |
| AC-3 | command + 관련 inspection | 존재하지 않는 context와 kubeconfig는 typed error다. | 미평가 | |
| AC-4 | command + 관련 inspection | test가 실제 사용자 kubeconfig를 읽지 않는다. | 미평가 | |
