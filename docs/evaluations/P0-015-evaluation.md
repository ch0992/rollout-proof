...
# P0-015 평가서: 410 Gone relist와 watch gap

## 1. 평가 계약

[작업지시서](../tasks/P0-015-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.

## 2. 평가 대상

- Work order version: 1
- Requirement ID: `ENG-WATCH-003`
- Issue/PR/commit SHA: 구현 후 기록
- 허용 주요 파일: `internal/kube/watch/relist.go`, `internal/kube/watch/relist_test.go`, `internal/kube/watch/cache.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 410 수신 시 fresh LIST 후 새 resourceVersion으로 watch한다. | 미평가 | |
| AC-2 | command + 관련 inspection | relist의 동일 object는 synthetic change를 만들지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | 변경/삭제 object만 domain event가 된다. | 미평가 | |
| AC-4 | command + 관련 inspection | gap 시작/종료와 relist 횟수가 diagnostic event에 기록된다. | 미평가 | |
| AC-5 | command + 관련 inspection | cache 상한 초과는 silent drop이 아닌 error다. | 미평가 | |
...
```bash
go test ./internal/kube/watch -run 'TestRelist|TestGap'
