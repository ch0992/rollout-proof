...
# P0-006 평가서: Event domain type 정의

## 1. 평가 목적

[P0-006 작업지시서](../tasks/P0-006-work-order.md)의 계약을 구현 설명과 독립적으로 검증한다. 검증 방법과 PASS 조건은 구현 전에 고정하며 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/timeline/event.go`, `internal/timeline/event_test.go`
...
|---|---|---|---|---|
| AC-1 | `go test ./internal/timeline -run 'Test.*Event'` 및 관련 code inspection | Event가 sequence, wall time, elapsed time, source, kind, object ref, typed payload를 표현한다. | 미평가 | |
| AC-2 | `go vet ./internal/timeline` 및 관련 code inspection | 임의 `map[string]any` payload를 public API가 받지 않는다. | 미평가 | |
| AC-3 | `go vet ./internal/timeline` 및 관련 code inspection | 필수 field가 없는 event 생성은 typed error를 반환한다. | 미평가 | |
| AC-4 | `go vet ./internal/timeline` 및 관련 code inspection | unit test가 유효/무효 constructor 경계를 검증한다. | 미평가 | |
