...
work_order: ../tasks/P0-027-work-order.md
evaluation_version: 1
verdict: null
---

# P0-027 평가서: Canonical timeline ordering

## 1. 평가 계약

[작업지시서](../tasks/P0-027-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/timeline/aggregate.go`, `internal/timeline/aggregate_test.go`, `internal/timeline/testdata/mixed.golden.json`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | output sequence가 유일하고 strictly increasing이다. | 미평가 | |
| AC-2 | command + 관련 inspection | 동일 입력은 동일 golden timeline을 만든다. | 미평가 | |
| AC-3 | command + 관련 inspection | Kubernetes resourceVersion을 전역 순서로 사용하지 않는다. | 미평가 | |
| AC-4 | command + 관련 inspection | source timestamp와 local receive time이 구분된다. | 미평가 | |
