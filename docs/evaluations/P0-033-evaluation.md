...
# P0-033 평가서: Readiness/Endpoint gap 결함 fixture

## 1. 평가 계약

[작업지시서](../tasks/P0-033-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `fixtures/endpoint-gap/deployment.yaml`, `fixtures/endpoint-gap/service.yaml`, `fixtures/endpoint-gap/app/main.go`, `fixtures/endpoint-gap/README.md`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 결함 원인이 manifest/app에 명시적이다. | 미평가 | |
| AC-2 | command + 관련 inspection | 정상 fixture와 한 가지 결함 축만 다르다. | 미평가 | |
| AC-3 | command + 관련 inspection | 반복 rollout에서 HTTP failure window가 발생한다. | 미평가 | |
| AC-4 | command + 관련 inspection | README가 예상 lifecycle과 reset 방법을 설명한다. | 미평가 | |
