...
status: planned
issue: https://github.com/ch0992/rollout-proof/issues/27
pull_request: null
evaluated_commit: null
work_order: ../tasks/P0-021-work-order.md
evaluation_version: 1
verdict: null
---

# P0-021 평가서: New-connection HTTP transport

## 1. 평가 계약

[작업지시서](../tasks/P0-021-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/probe/transport/new.go`, `internal/probe/transport/new_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 연속 요청에서 connection reuse가 0이다. | 미평가 | |
| AC-2 | command + 관련 inspection | HTTP/1.1으로 protocol이 고정된다. | 미평가 | |
| AC-3 | command + 관련 inspection | request context 취소가 socket을 종료한다. | 미평가 | |
| AC-4 | command + 관련 inspection | httptest listener가 새 connection 수를 검증한다. | 미평가 | |
