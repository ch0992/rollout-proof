...
# P0-036 평가서: Phase 0 결과와 Go/No-Go ADR

## 1. 평가 계약

[작업지시서](../tasks/P0-036-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `docs/experiments/phase-0-results.md`, `docs/adr/0009-phase-0-go-no-go.md`, `docs/README.md`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 각 Phase 0 gate에 artifact evidence가 링크된다. | 미평가 | |
| AC-2 | command + 관련 inspection | 성공/실패/INCONCLUSIVE run 수가 모두 기록된다. | 미평가 | |
| AC-3 | command + 관련 inspection | 결정된 parameter와 근거가 있다. | 미평가 | |
| AC-4 | command + 관련 inspection | 미달 기준은 후속 Task 또는 No-Go 이유로 연결된다. | 미평가 | |
| AC-5 | command + 관련 inspection | 문서 인덱스에 결과와 ADR이 연결된다. | 미평가 | |
