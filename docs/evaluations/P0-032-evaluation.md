...
# P0-032 평가서: 정상 Rollout fixture

## 1. 평가 계약

[작업지시서](../tasks/P0-032-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `fixtures/normal/deployment.yaml`, `fixtures/normal/service.yaml`, `fixtures/normal/app/main.go`, `fixtures/normal/README.md`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | fixture가 health와 backend identity endpoint를 제공한다. | 미평가 | |
| AC-2 | command + 관련 inspection | readiness 전 traffic을 받지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | SIGTERM 후 graceful drain을 수행한다. | 미평가 | |
| AC-4 | command + 관련 inspection | revision update가 deterministic하다. | 미평가 | |
| AC-5 | command + 관련 inspection | kind에서 rollout이 Available 상태로 완료된다. | 미평가 | |
