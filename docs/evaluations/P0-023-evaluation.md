...
# P0-023 평가서: HTTP 단계별 오류 classifier

## 1. 평가 계약

[작업지시서](../tasks/P0-023-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/probe/classify/error.go`, `internal/probe/classify/error_test.go`, `internal/probe/classify/testserver_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 정의한 오류 category table test가 모두 통과한다. | 미평가 | |
| AC-2 | command + 관련 inspection | 오류 문자열만으로 category를 결정하지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | context cancellation과 request deadline을 구분한다. | 미평가 | |
| AC-4 | command + 관련 inspection | 원인 error wrapping이 유지된다. | 미평가 | |
| AC-5 | command + 관련 inspection | 미분류 오류는 unknown이지 success가 아니다. | 미평가 | |
