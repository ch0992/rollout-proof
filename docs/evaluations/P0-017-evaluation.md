...
status: planned
issue: https://github.com/ch0992/rollout-proof/issues/23
pull_request: null
evaluated_commit: null
work_order: ../tasks/P0-017-work-order.md
evaluation_version: 1
verdict: null
---

# P0-017 평가서: EndpointSlice watch adapter

## 1. 평가 계약

[작업지시서](../tasks/P0-017-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/kube/watch/endpointslice.go`, `internal/kube/watch/endpointslice_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 대상 Service label이 없는 slice를 제외한다. | 미평가 | |
| AC-2 | command + 관련 inspection | endpoint add/remove를 안정적 identity로 표현한다. | 미평가 | |
| AC-3 | command + 관련 inspection | ready/serving/terminating nil 의미를 보존한다. | 미평가 | |
| AC-4 | command + 관련 inspection | 주소와 targetRef 외 불필요한 metadata를 report payload에 넣지 않는다. | 미평가 | |
