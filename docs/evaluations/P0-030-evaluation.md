...
# P0-030 평가서: Observe prototype command wiring

## 1. 평가 계약

[작업지시서](../tasks/P0-030-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/app/observe/run.go`, `internal/app/observe/run_test.go`, `internal/cli/observe.go`, `internal/cli/observe_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | observe command가 target과 URL을 validation한다. | 미평가 | |
| AC-2 | command + 관련 inspection | fake adapter event가 terminal과 JSON에 동일하게 반영된다. | 미평가 | |
| AC-3 | command + 관련 inspection | timeout/cancel이 pipeline까지 전파된다. | 미평가 | |
| AC-4 | command + 관련 inspection | adapter 오류가 non-zero exit category로 전달된다. | 미평가 | |
| AC-5 | command + 관련 inspection | test가 실제 network/cluster를 요구하지 않는다. | 미평가 | |
