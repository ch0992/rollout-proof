...
# P0-005 평가서: ADR-0002 Go 단일 프로세스 결정 기록

## 1. 평가 목적

[P0-005 작업지시서](../tasks/P0-005-work-order.md)의 계약을 구현 설명과 독립적으로 검증한다. 검증 방법과 PASS 조건은 구현 전에 고정하며 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `docs/adr/0002-single-process-cli.md`, `docs/README.md`
...
|---|---|---|---|---|
| AC-1 | `test -f docs/adr/0002-single-process-cli.md` 및 관련 code inspection | ADR 번호가 기존 ADR과 충돌하지 않는다. | 미평가 | |
| AC-2 | `rg -n 'Context|Decision|Alternatives|Consequences' docs/adr/0002-single-process-cli.md` 및 관련 code inspection | Go 단일 binary와 기본 cluster 설치물 없음이 명시된다. | 미평가 | |
| AC-3 | `rg -n '0002-single-process-cli' docs/README.md` 및 관련 code inspection | controller 방식과 runtime 방식의 trade-off가 기록된다. | 미평가 | |
| AC-4 | `rg -n '0002-single-process-cli' docs/README.md` 및 관련 code inspection | 문서 인덱스에서 ADR을 찾을 수 있다. | 미평가 | |
