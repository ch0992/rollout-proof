...
work_order: ../tasks/P0-016-work-order.md
evaluation_version: 1
verdict: null
---

# P0-016 평가서: Pod lifecycle watch adapter

## 1. 평가 계약

[작업지시서](../tasks/P0-016-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/kube/watch/pod.go`, `internal/kube/watch/pod_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 관련 owner UID가 아닌 Pod를 제외한다. | 미평가 | |
| AC-2 | command + 관련 inspection | Ready condition 변화가 typed event로 변환된다. | 미평가 | |
| AC-3 | command + 관련 inspection | deletionTimestamp와 termination reason을 보존한다. | 미평가 | |
| AC-4 | command + 관련 inspection | 민감 env/spec 전체를 payload에 포함하지 않는다. | 미평가 | |
