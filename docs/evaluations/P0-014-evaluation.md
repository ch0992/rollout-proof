...
# P0-014 평가서: WATCH EOF reconnect와 backoff

## 1. 평가 계약

[작업지시서](../tasks/P0-014-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.

## 2. 평가 대상

- Work order version: 1
- Requirement ID: `ENG-WATCH-002`
- Issue/PR/commit SHA: 구현 후 기록
- 허용 주요 파일: `internal/kube/watch/reconnect.go`, `internal/kube/watch/reconnect_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | EOF 후 마지막 resourceVersion으로 재연결한다. | 미평가 | |
| AC-2 | command + 관련 inspection | backoff가 250ms에서 시작해 10s를 넘지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | fake clock/random source로 test가 결정적이다. | 미평가 | |
| AC-4 | command + 관련 inspection | 인증/권한 오류는 무한 retry하지 않는다. | 미평가 | |
...
```bash
go test ./internal/kube/watch -run TestReconnect
