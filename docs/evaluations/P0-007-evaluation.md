...
work_order: ../tasks/P0-007-work-order.md
evaluation_version: 1
verdict: null
---

# P0-007 평가서: Real/Fake clock과 elapsed time 구현

## 1. 평가 목적

[P0-007 작업지시서](../tasks/P0-007-work-order.md)의 계약을 구현 설명과 독립적으로 검증한다. 검증 방법과 PASS 조건은 구현 전에 고정하며 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `internal/timeline/clock.go`, `internal/timeline/clock_test.go`
...
|---|---|---|---|---|
| AC-1 | `go test ./internal/timeline -run 'Test.*Clock'` 및 관련 code inspection | real clock이 UTC wall time과 run-start elapsed를 제공한다. | 미평가 | |
| AC-2 | `rg -n 'time\.Sleep' internal/timeline/*_test.go && exit 1 || true` 및 관련 code inspection | fake clock을 test에서 결정적으로 전진시킬 수 있다. | 미평가 | |
| AC-3 | `go test -race ./internal/timeline` 및 관련 code inspection | fake clock test가 `time.Sleep` 없이 elapsed 값을 검증한다. | 미평가 | |
| AC-4 | `go test -race ./internal/timeline` 및 관련 code inspection | clock 역행 입력은 음수 elapsed를 노출하지 않는다. | 미평가 | |
