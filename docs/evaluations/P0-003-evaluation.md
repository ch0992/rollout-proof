...
# P0-003 평가서: Cobra root command와 출력 channel 분리

## 1. 평가 목적

[P0-003 작업지시서](../tasks/P0-003-work-order.md)의 계약을 구현 설명과 독립적으로 검증한다. 검증 방법과 PASS 조건은 구현 전에 고정하며 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `go.mod`, `cmd/rollout-proof/main.go`, `internal/cli/root.go`, `internal/cli/root_test.go`
...
|---|---|---|---|---|
| AC-1 | `go test ./internal/cli ./cmd/rollout-proof` 및 관련 code inspection | `rollout-proof --help`에 제품명과 command가 표시된다. | 미평가 | |
| AC-2 | `go run ./cmd/rollout-proof version` 및 관련 code inspection | `rollout-proof version`이 stdout에 version을 출력하고 exit 0이다. | 미평가 | |
| AC-3 | `go run ./cmd/rollout-proof unknown-command` 및 관련 code inspection | 잘못된 command는 non-zero이며 오류가 stderr로만 출력된다. | 미평가 | |
| AC-4 | `go run ./cmd/rollout-proof unknown-command` 및 관련 code inspection | test가 process spawn 없이 출력 channel을 검증한다. | 미평가 | |
