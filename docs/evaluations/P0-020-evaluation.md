...
work_order: ../tasks/P0-020-work-order.md
evaluation_version: 1
verdict: null
---

# P0-020 평가서: Bounded probe scheduler

## 1. 평가 계약

[작업지시서](../tasks/P0-020-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.

## 2. 평가 대상

- Work order version: 1
- Requirement ID: `ENG-PROBE-002`
- Issue/PR/commit SHA: 구현 후 기록
- 허용 주요 파일: `internal/probe/scheduler.go`, `internal/probe/scheduler_test.go`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | fake clock으로 지정 interval job을 만든다. | 미평가 | |
| AC-2 | command + 관련 inspection | queue가 가득 차도 무한 backlog를 만들지 않는다. | 미평가 | |
| AC-3 | command + 관련 inspection | lag/drop을 diagnostic result로 노출한다. | 미평가 | |
| AC-4 | command + 관련 inspection | context 취소 후 worker가 종료된다. | 미평가 | |
| AC-5 | command + 관련 inspection | race detector가 통과한다. | 미평가 | |
...
```bash
go test ./internal/probe -run TestScheduler
