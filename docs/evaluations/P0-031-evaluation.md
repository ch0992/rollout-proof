...
# P0-031 평가서: Kind cluster bootstrap

## 1. 평가 계약

[작업지시서](../tasks/P0-031-work-order.md)의 검증 방법과 PASS 조건은 구현 전에 고정한다. 구현 후에는 결과와 evidence만 기록한다.
...
- 허용 주요 파일: `test/e2e/kind/Makefile`, `test/e2e/kind/cluster.yaml`, `test/e2e/kind/README.md`

## 3. Acceptance Criteria 검증표

| AC | 검증 방법 | PASS 조건 | 결과 | Evidence |
|---|---|---|---|---|
| AC-1 | command + 관련 inspection | 지정 Kubernetes node image로 cluster를 만든다. | 미평가 | |
